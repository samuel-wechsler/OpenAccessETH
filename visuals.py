...
# Plot A: Using broken axes for the first subplot
bax1 = brokenaxes(ylims=((0, 800), (3200, 4500)),
                  subplot_spec=gs[0], hspace=.1, despine=False)
bax1.grid(True, which='both', linestyle='--',
          linewidth=0.5, alpha=0.5, zorder=0)
bax1.tick_params(axis='x', rotation=45)
bax1.set_xlabel("Year", labelpad=30)
bax1.set_ylabel("Total Number of Lecture Series at ETH", labelpad=40)

bax1.bar(catalogue_data['year'],
         catalogue_data['total_count'],
         color=colors['catalogue'],
         edgecolor='black',
         label='Course Catalogue')

bax1.bar(portal_year_data['year'],
         portal_year_data['total_count'],
         color=colors['portal'],
         edgecolor='black',
...
bax1.legend(loc='upper left', frameon=False)

# Plot B: Fraction of lectures recorded and publicly uploaded
ax2 = fig.add_subplot(gs[1])
ax2.grid(True, which='both', linestyle='--',
         linewidth=0.5, alpha=0.5, zorder=0)
ax2.set_xlabel("Year", labelpad=0)
ax2.set_ylabel(
    "Fraction of Respective Lecture Type", labelpad=10)
ax2.tick_params(axis='x', rotation=45)

ax2.bar(catalogue_data['year'],
